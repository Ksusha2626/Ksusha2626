...
# print(timeit.timeit('err(80)', number=100, globals=globals()))  # 79.9881973
...
"""
1 алгоритм вероятнее всего имеет квадратичную сложность. Формируется массив константной длины за счет чего расходуется
много памяти, а для поиска числа перебираются все числа в массиве пока не будет обнаружено нужное что так же сказывается 
на производительности. Отсюда и показатели скорости при большом N, для поиска 80 простого числа
алгоритм затрачивает почти 1.5 минуты.

2 алгоритм скорее всего имеет логарифмическую сложность. Данный алгоритм генерируется массив из простых чисел только лишь
в диапозоне 0 до N, что существенно экономит памят и позволяет найти число гораздо быстрее. 80 число в массиве найдено
за милисекунды. Правда смущают данные по отчету профайла: кажется, функция len создает чересчур много объектов. Возможно,
от нее следует избавится в пользу более лучшего решения. 
